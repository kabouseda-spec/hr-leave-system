...
router.post('/', auth, (req, res) => {
  ...
  const { relationship, name, date_of_birth, gender, in_uae } = req.body;
  if (!relationship || !name) return res.status(400).json({ error: 'relationship and name are required' });

  const id = uuidv4();
  db.prepare(
    'INSERT INTO family_members (id, employee_id, relationship, name, date_of_birth, gender, in_uae) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run(id, req.params.id, relationship, name.trim(), n(date_of_birth), n(gender), in_uae === false || in_uae === 0 ? 0 : 1);

  res.status(201).json({ id, relationship, name: name.trim(), date_of_birth: n(date_of_birth), gender: n(gender), in_uae: in_uae === false || in_uae === 0 ? 0 : 1 });
});

// PATCH /api/employees/:id/family/:memberId
router.patch('/:memberId', auth, (req, res) => {
  if (!canAccess(req, req.params.id, true)) return res.status(403).json({ error: 'Access denied' });
  const { name, date_of_birth, relationship, in_uae } = req.body;
  db.prepare(`UPDATE family_members SET
    name = COALESCE(?, name),
    relationship = COALESCE(?, relationship),
    date_of_birth = COALESCE(?, date_of_birth),
    in_uae = CASE WHEN ? IS NOT NULL THEN ? ELSE in_uae END
    WHERE id = ? AND employee_id = ?`)
    .run(n(name), n(relationship), n(date_of_birth),
         in_uae !== undefined ? 1 : null, in_uae !== undefined ? (in_uae ? 1 : 0) : null,
